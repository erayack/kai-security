...
async def test_adapter_chooser_detects_rust_and_cmake_without_llm(
    ...
    monkeypatch.setattr(
        adapter_chooser, "get_model_pricing", lambda *a, **k: {"prompt": 0.0, "completion": 0.0}
    )
    ...
    out = await proc.execute(AdapterChooserInput(model_name="__no_model__", use_openai=False))
