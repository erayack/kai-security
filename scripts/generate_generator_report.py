...
def analyze_validation_conversations(output_dir: str, repo_slug: str) -> Dict[str, Any]:
    ...
    if not conv_files:
        return {"error": f"No conversation files found in {validation_dir}"}
        ...
        try:
            ...
            if exception_occurred:
                ...
                verified_exploits.append(
                    {**exploit_info, "conversation_file": conv_file.name}
                )
            else:
                failed_count += 1
                severity_stats[severity]["unverified"] += 1
    ...
    verification_rate = (
        (verified_count / total_exploits * 100) if total_exploits > 0 else 0.0
    )
        ...
        "severity_breakdown": {
            severity: {
                ...
                "verification_rate": round(
                    stats["verified"] / (stats["verified"] + stats["unverified"]) * 100,
                    1,
                )
                if (stats["verified"] + stats["unverified"]) > 0
                else 0.0,
            }
            for severity, stats in sorted(severity_stats.items())
        },
        "efficiency_metrics": {
            "cost_per_verified_exploit": round(total_cost / verified_count, 4)
            if verified_count > 0
            else 0,
            "time_per_verified_exploit_seconds": round(total_time / verified_count, 2)
            if verified_count > 0
            else 0,
            "time_per_verified_exploit": format_time(total_time / verified_count)
            if verified_count > 0
            else "0s",
            "verified_per_minute": round(verified_count / (total_time / 60), 2)
            if total_time > 0
            else 0,
            "exploits_per_dollar": round(total_exploits / total_cost, 2)
            if total_cost > 0
            else 0,
            "verified_per_dollar": round(verified_count / total_cost, 2)
            if total_cost > 0
            else 0,
        },
...
def save_report(
    ...
    output_dir: str,
    repo_slug: str,
    filename: str = "generator_report.json",
) -> str:
...
def main():
    ...
    for severity, stats in report["severity_breakdown"].items():
        print(
            f"  {severity.upper()}: {stats['verified']}/{stats['total']} verified ({stats['verification_rate']}%)"
        )

    print(f"\nEfficiency Metrics:")
    print(
        f"  Cost per verified: ${report['efficiency_metrics']['cost_per_verified_exploit']}"
    )
    print(
        f"  Time per verified: {report['efficiency_metrics']['time_per_verified_exploit']}"
    )
    print(
        f"  Verified per minute: {report['efficiency_metrics']['verified_per_minute']}"
    )
    print(
        f"  Verified per dollar: {report['efficiency_metrics']['verified_per_dollar']}"
    )
