...
class DependencyGraph:
    ...
    def __init__(self, root_dir: str | Path) -> None:
        ...
        self._container_by_name: Dict[str, Set[str]] = defaultdict(set)
    ...
    def add_node(self, node: Node) -> None:
        ...
        if node.kind == NodeKind.FILE and file_path:
            ...
            self._container_by_name[node.name].add(node.id)
        elif node.kind == NodeKind.UNIT:
    ...
    def edges(
        self,
    ) -> Iterator[Tuple[str, EdgeKind, str, Dict[str, Any]]]:
    ...
    def find_containers(self, name: str) -> List[str]:
        ...
        return sorted(self._container_by_name.get(name, set()))

    def find_units(self, name: str) -> List[str]:
        """Find unit node IDs by name."""
    ...
    def bfs(
        ...
        *,
        ...
        start_list = [s for s in start if s in self._nodes]
        seen: Set[str] = set(start_list)
        q: deque[Tuple[str, int]] = deque((s, 0) for s in start_list)
    ...
    def derive_related_files(
        ...
        *,
        ...
        Modes:
          ...
          REAL_SOURCE — imports + inheritance + calls
    ...
    def get_edge_meta(
        ...
        if kind:
            return self._edges.get((src, kind, dst))
        for (s, k, d), em in self._edges.items():
            if s == src and d == dst:
                return em
        return None
