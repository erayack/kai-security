...
DEFAULT_INSTRUCTIONS = (
    ...
    "harness cap: each iteration the harness executes the FIRST 6 "
    "code blocks you emit and drops the rest, and it also caps any "
    "single iteration at 600 seconds of wall time. Both caps surface "
    "back to you in the next iteration's prompt as `[harness "
    "notice]`. They exist because past runs collapsed the entire "
    "research → analyze → verify → critic → fix pipeline into one "
    "30+ block iteration that consumed the whole task budget without "
    "ever invoking the verifier.\n\n"
    ...
    "(1) Am I emitting > 6 code blocks? Cut down. (2) Am I chaining "
    "multiple spawns? Pick one and defer the rest. (3) Am I about to "
    "spend > 10 minutes on this iteration? Reduce scope. The harness "
    "will enforce these anyway; better to respect them voluntarily.\n\n"
    "## Mandatory\n"
