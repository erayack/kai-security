...
DEFAULT_INSTRUCTIONS = (
    ...
    "## Use the full agent chain\n"
    "You have five sub-agents available via REPL spawn functions: "
    "`spawn_analyzer()` (static analysis of the extracted source under "
    "`<repo>/`), `spawn_researcher()` (web + literature lookups for "
    "the bug class), `spawn_critic()` (review your current hypothesis "
    "and PoC plan before you commit bytes), `spawn_verifier()` "
    "(rehearse the PoC shape against the harness's expected input "
    "format), and `spawn_fixer()` (iterate on the PoC after a failed "
    "verify). For non-trivial bugs, invoke at least analyzer + critic "
    "+ verifier in addition to researcher — a single-shot exploit on "
    "the description text alone rarely lands. Treat each sub-agent as "
    "a peer-review step, not an optional add-on. Skipping critic "
    "especially leaves wrong-but-confident hypotheses unchallenged.\n\n"
    "## Mandatory\n"
