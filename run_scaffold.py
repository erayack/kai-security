...
def _repo_slug(repo_url: str) -> str:
    ...
    short_hash = (
        _get_repo_commit_hash(repo_url)
        or hashlib.sha1(repo_url.encode("utf-8")).hexdigest()[:8]
    )
    ...
    if response and not exception_occurred:
        ...
        if not done_present:
            ...
            if not exception_msg:
                exception_msg = (
                    "Setup agent did not complete successfully (no <done> tag found)"
                )
...
async def run_generator_agent(
    ...
    if not os.path.exists(repo_path):
        raise ValueError(
            f"Repository not found at {repo_path}. Setup agent should have cloned it."
        )
...
async def run_generator_validation(
    ...
    # Split into batches
    batches = [
        exploits_files[i : i + GENERATOR_BATCH_SIZE]
    ...
    success_rate = (
        (verified_exploits / total_exploits * 100) if total_exploits > 0 else 0.0
    )
    ...
    try:
        output_dir = os.path.join(_project_root(), "output")
        report = generate_generator_report(repo_slug=repo_slug, output_dir=output_dir)
        report_path = save_report(
            report=report, output_dir=output_dir, repo_slug=repo_slug
        )
...
async def run_fixer_agent(
    ...
    if not os.path.exists(repo_path):
        ...
        raise ValueError(
            f"Repository not found at {repo_path}. Finder agent should have cloned it."
        )
    # Find all exploits.json files
    print("\n🔍 Searching for exploits.json files...")
    exploits_files = get_exploits_jsons(repo_path)
    ...
    # Calculate success rate
    success_rate = (
        (fixed_exploits / total_exploits * 100) if total_exploits > 0 else 0.0
    )
    ...
    try:
        output_dir = os.path.join(_project_root(), "output")
        report = generate_fixer_report(repo_slug=repo_slug, output_dir=output_dir)
        report_path = save_report(
            report=report, output_dir=output_dir, repo_slug=repo_slug
        )
        print(f"   Report saved to: {report_path}")
...
Examples:
  ...
  python run_scaffold.py --turns 50
    ...
    parser.add_argument(
        "--use-openai", action="store_true", help="Use OpenAI API instead of OpenRouter"
    )

    # Agent selection (mutually exclusive group for convenience)
    agent_group = parser.add_mutually_exclusive_group()
    agent_group.add_argument(
        "--setup-only", action="store_true", help="Run only the setup agent"
    )
    agent_group.add_argument(
        "--finder-only",
        action="store_true",
    ...
    parser.add_argument(
        "--skip-setup", action="store_true", help="Skip the setup agent"
    )
    parser.add_argument(
        "--skip-finder", action="store_true", help="Skip the finder agent"
    )
    parser.add_argument(
        "--skip-generator", action="store_true", help="Skip the generator agent"
    )
    parser.add_argument(
        "--skip-fixer", action="store_true", help="Skip the fixer agent"
    )
    ...
    run_finder = True
    run_generator = True
    run_fixer = True  # Added fixer to default run
    ...
    print(
        f"Agents to run: {', '.join([a for a, enabled in [('Finder', run_finder), ('Setup', run_setup), ('Generator', run_generator), ('Fixer', run_fixer)] if enabled])}"
    )
    print("=" * 80)
    ...
    if run_finder:
        print("\n" + "=" * 80)
        print(
            f"STEP {step_num}/{total_steps}: Running Finder Agent (Model: {model_name})"
        )
        print("=" * 80)
    ...
    if run_setup:
        print("\n" + "=" * 80)
        print(
            f"STEP {step_num}/{total_steps}: Running Setup Agent (Model: {setup_model})"
        )
        print("=" * 80)
        ...
        print(
            f"✅ Setup agent completed successfully. Cost: ${setup_result['estimated_cost']:.4f}"
        )
    ...
    if run_generator:
        print("\n" + "=" * 80)
        print(
            f"STEP {step_num}/{total_steps}: Running Generator Validation (Model: {model_name})"
        )
        print("=" * 80)
    ...
    if run_fixer:
        print("\n" + "=" * 80)
        print(
            f"STEP {step_num}/{total_steps}: Running Fixer Agent (Model: {model_name})"
        )
        print("=" * 80)
        fixer_result = await run_fixer_agent(
