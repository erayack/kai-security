...
async def process_exploits_json(
    ...
    try:
        ...
        async def validate_single_exploit(exploit):
            ...
            # Check if 'test' or 'tests' exists
            if os.path.exists(os.path.join(repo_path, "test")):
                test_dir_name = "test"
            else:
                test_dir_name = "tests"

            exploits_test_dir = os.path.join(repo_path, test_dir_name, "exploits")
...
4. Create a test file in `{test_dir_name}/exploits/` (e.g. `{test_dir_name}/exploits/test_<id>.t.sol` or `{test_dir_name}/exploits/test_<id>.rs`) that PASSES if the exploit exists.
5. Run the test targeting that specific file (e.g. `forge_test(test_script_path="{test_dir_name}/exploits/test_<id>.t.sol")`).
...
IMPORTANT: When writing regex strings in Python, DO NOT use single quotes inside a single-quoted raw string (e.g. r'foo'bar'). This causes SyntaxError.
Use double quotes for the outer string if the inner regex contains single quotes (e.g. r"foo'bar"), or escape the single quotes.
"""
        ...
        for coro in asyncio.as_completed(tasks):
            ...
            if test_passed and not exception_occurred:
                ...
                log_exploit_verified(
                    exploit_id=exploit_id, verified_by_agent_id=result["sub_agent_id"]
                )
            else:
        ...
        return {
            ...
            "success_rate": (
                verified_count / total_exploits if total_exploits > 0 else 0.0
            ),
            "details": details,
